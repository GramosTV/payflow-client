...
export class QrCodeComponent implements OnInit, OnDestroy {
    ...
    this.qrCodeService
      ...
      .subscribe({
        next: response => {
          ...
          this.expirationTime = response.expiresAt ? new Date(response.expiresAt) : undefined;
  ...
  shareQRCode(): void {
    if (!this.qrCodeData || !navigator.share) {
      this.errorHandler.showErrorMessage('Sharing is not supported on this device');
      return;
    }
  ...
  startScanning(): void {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      this.errorHandler.showErrorMessage('Camera access is not supported in your browser');
      return;
    }
    ...
    if (this.videoElement && this.canvasElement) {
      this.scannerService
        .startScanner(this.videoElement.nativeElement, this.canvasElement.nativeElement)
        .then(success => {
          if (success) {
  ...
  makePayment(): void {
    if (!this.scanResult || (this.scanResult.amount === undefined && !this.customAmount)) {
    ...
    if (this.paymentMethodId === 'WALLET' && this.walletBalance < paymentAmount!) {
    ...
    this.qrCodeService
      .processQRCodePayment({
        ...
        paymentMethodId: this.paymentMethodId === 'WALLET' ? undefined : this.paymentMethodId,
   ...
   */
  private loadQrCodeImage(qrCodeId: number, retryCount = 0, maxRetries = 3, delay = 1000): void {
    ...
    this.qrCodeService.getQRCodeImageById(qrCodeId).subscribe({
      ...
      error: error => {
        console.error(`Error loading QR code image (attempt ${retryCount + 1}):`, error);
        this.isLoadingQrImage = false;

        // If we haven't reached max retries, try again after delay
        if (retryCount < maxRetries) {
          this.qrCodeImageError = `Loading image (retry ${retryCount + 1}/${maxRetries})...`;
          setTimeout(() => {
            this.loadQrCodeImage(qrCodeId, retryCount + 1, maxRetries, delay);
          }, delay);
        } else {
          this.qrCodeImageError = 'Failed to load QR code image. Please try again.';
